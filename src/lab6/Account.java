...
public class Account {
	...
	// TODO: Task3 
	// replace the null with a lambda expression
	public static AddMaker maker = a -> t -> t.balance+=a ;
	...
	public static int getMaxAccountID(List<Account> accounts) {
		// TODO: Task4 
		// replace the null with a lambda expression
		Account maxOne = accounts.stream().reduce(new Account(0, -100), (max, now)-> {
			if (max.balance< now.balance) {
				max.id=now.id;
				max.balance=now.balance;
			}
			return max;
		});
