...
public class NoteBookWindow extends Application {
	...
	private HBox addHBox() {
		...
		hbox.getChildren().addAll(buttonLoad, buttonSave, searchLabel, searchText, searchButton, clearSearch);
		...
		for (Folder folder: noteBook.getFolders()) {
			if (folder.getName().equals(currentFolder)) {
				if (currentSearch.equals("")) {
					...
					ArrayList<Note> noteList = folder.searchNotes(currentSearch);
						...
						if (f.getName().equals(currentFolder)) {
							for (Note n: f.getNotes()) {
								if (n.getTitle().equals(currentNote)) {
									n = new TextNote(currentNote,textAreaNote.getText());
									break;
								}
							}
							break;
